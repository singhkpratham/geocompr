...
Note that the functions we created are called recursively in `lapply()` and `vapply()` function calls respectively, which you may want to read-up on in `?lapply` for reference.
]
...
Not only have we discussed them in the abstract.
We have created working examples of each to solve a specific problem:
...
If you are new to programming, as we expect most people reading this book will be, being able to follow and reproduce the results in the preceding sections should be seen as a major acheivement. 
As outlined in the introduction programming takes many hours of dedicated study and practice before you become proficient.
...
A question arising from this is: how would one generalise the function.

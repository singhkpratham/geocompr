...
To learn more about **RQGIS**\index{RQGIS (package)}, please refer to @muenchow_rqgis:_2017. 
...
So the function call for calculating the 'SAGA Wetness Index'\index{wetness index} becomes as simple as:
...
- Please note that we have used GRASS's spatial database\index{spatial database} (based on SQLite) which allows faster processing. 
...
This includes duplicated functionality, e.g., you can perform overlay-operations using QGIS-\index{QGIS}, SAGA-\index{SAGA} or GRASS-geoalgorithms\index{GRASS}.
...
- **RQGIS** can also handle spatial objects residing in R as input for geoalgorithms\index{geoalgorithm}, and loads QGIS output automatically back into R if desired.
...
Please note that **rgrass7**\index{rgrass7 (package)} inspired the latter two features.
...
1. Compute catchment area\index{catchment area} and catchment slope of `data("dem", package = "RQGIS")` using **RSAGA** (see Section \@ref(rsaga)).

...
The difference is that zonal filters can take on any shape instead of a predefined rectangular window, i.e., the raster cells defining the filter do not have to be necessarily neighors.
...
To find the mean elevation for each grain size class (left panel of Figure \@ref(fig:cont-raster)), we can use the `zonal()` command.
